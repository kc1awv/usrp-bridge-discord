...
use usrp_bridge_discord::USRPVoicePacketType;
...
impl Transmitter {
    pub fn new() -> Self {
        ...
        let usrp_target_tx_addr = env::var("USRP_SEND")
            .expect("Expected a target tx address in the environment");
        let socket = UdpSocket::bind("0.0.0.0:0")
            .expect("Couldn't bind udp socket for transmission");
        socket
            .connect(usrp_target_tx_addr)
            .expect("Couldn't connect to USRP's audio receiver");
            ...
            Ctx::SpeakingUpdate(data) => {
                if data.speaking {
                    ...
                    self.tx
                        ...
                        .expect("Couldn't send discord's audio packet through USRP transmitter");
                } else {
